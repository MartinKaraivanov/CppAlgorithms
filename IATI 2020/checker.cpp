...
{if (parCnt>1) return -1;//Command has no params
 char in[256];
 long long r=0;
 if (readNext(inp,in)!=2) return -2;//Wrong input
...
{if (parCnt>1) return -1;//Command has no params
 char c[256];
 if (readNext(inp,c)!=1) return -2;//Wrong input
 PF.IN=*c;
 charIn=1;
 currRow++;
 return 0;
...
{char row[8][32];
 ...
 {case 0: {return 1;}//{showPF();return 1;}
  case -10:{return 2;}//{showPF();return 2;}
...
int interprete(const char *fname,const char *infname,long long *res)
{char b[64];
 *res=0;
 switch(readProg(fname))
 ...
 {if (currRow>=cnt){fprintf(stderr,"Missing line\n");fclose(inp);return -1;}
  ...
  if (n==2){fprintf(stderr,"Execution successful\n");
            *res=PF.A;
            fclose(inp);
            return 0;
           }
  if (!n) {fclose(inp);return 0;}
 }while(1);
 return 0;
}

char buf[2048];
long long author,comp;
int er;
int main(int c,char **s)
{
 {ifstream f("PF.txt");//contestant file
  if (!f){cout<<"0\n";cerr<<"Solution not found\n";return 0;}
  int n;
  for (n=0;!f.eof();n++) f.getline(buf,2047);
  if (n>255){cout<<"0\n";cerr<<"Too many program lines\n";return 0;}
 }
 {ifstream f("PF.20.in"); //in-file
  if (!f){cout << "0\n";cerr<<"In-file "<<"PF.01.in"<<" not found\n";return 0;}
 }
 {ifstream f("PF.20.sol");//sol-file
  if (!f){cout << "0\n";cerr<<"Sol-file "<<"PF.01.sol"<<" not found\n";return 0;}
  f>>author;
 }
 er=interprete("PF.txt","PF.20.in",&comp);
 if (er) {
     cout << "0\n";
     return 0;
 }
 if (comp!=author) {cout<<"0\n"; cerr<<"Wrong result\n";}
 else {cout<<"1\n"; cerr<<"Correct\n";}
 return 0;
}
